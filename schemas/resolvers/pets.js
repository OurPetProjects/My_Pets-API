...
const Pets = {
  ...
  Mutation: {
    // ! Create Pet
    async createPet(_, { petName, species, age, parentUsername, notes }) {
    ...
    async deletePet(_, { petId }) {
      ...
      try {
        if (pet) {
          ...
          return removePet;
        } else throw new AuthenticationError("You can not delete this pet.");
      } catch (error) {
    ...
    async editPet(
      parent,
      { petId, petName, species, age, parentUsername, notes }
    ) {
